...
<body class="flex justify-center items-center min-h-screen bg-slate-500">
    <?php
        ...
        if(isset($_GET["id_country"]) && !isset($_GET["id_city"]) && !isset($_GET["add"])){
            ...
            $res = $res->fetch_assoc();
            echo '<form id="countryForm" class="space-y-4 font-[sans-serif] max-w-md mx-auto bg-slate-300 py-10 px-10" action="edit.php?id_country='.$id_country.'" method="POST">
            <span class="font-bold text-xl"> Edit Country</span>
        <input type="text" placeholder="name" class="px-4 py-3 bg-gray-100 w-full text-sm outline-none border-b-2 border-blue-500 rounded" name="name" value="'.$res["name"].'" required/>
        ...
        if(isset($_GET["id_country"]) && isset($_GET["id_city"]) && !isset($_GET["add"])){
            ...
            $res = $mysqli->query($sql);
            $res = $res->fetch_assoc();
            echo '<form id="cityForm" class="space-y-4 font-[sans-serif] max-w-md mx-auto bg-slate-300 py-10 px-10" action="edit.php?id_country='.$id_country.'&id_city='.$id_city.'" method="POST">
        ...
        //add country form
        if(isset($_GET["add"])){
            echo '<form id="addCountryForm" class="space-y-4 font-[sans-serif] max-w-md mx-auto bg-slate-300 py-10 px-10" action="" method="POST">
